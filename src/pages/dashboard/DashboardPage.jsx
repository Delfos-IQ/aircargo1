...
export default function DashboardPage() {
  ...
  const awbAlertCount = React.useMemo(() => {
    if (!isAdmin) return 0;
    return (awbStockAllocations || []).filter(a => {
      const total = (a.endNumber && a.startNumber)
        ? parseInt(a.endNumber.substring(0, 7), 10) - parseInt(a.startNumber.substring(0, 7), 10) + 1
        : 0;
      const used = (a.usedAwbs || []).length;
      const pct  = total > 0 ? (total - used) / total : 0;
      return pct < 0.25 || used >= total;
    }).length;
  }, [awbStockAllocations, isAdmin]);

  const stats = [
    { label: isAdmin ? 'Total bookings' : 'My bookings', value: myBookings?.length ?? '…', color: 'blue',
  ...
  const allModules = [
    ...
    { title: 'AWB Stock', path: '/awb-stock', color: 'amber', roles: ['admin', 'operator'], description: 'Manage AWB stock batches and agent allocations',
    ...
    { title: 'Billing', path: '/billing', color: 'indigo', roles: ['admin'], description: 'Generate agent invoices with PDF export',
    ...
    { title: 'Agents', path: '/agents', color: 'purple', roles: ['admin', 'operator'], description: 'Freight agent profiles with IATA/CASS data',
    ...
    { title: 'Flights', path: '/flights', color: 'rose', roles: ['admin', 'operator'], description: 'Flight schedules, routes, operating days and maximum payload',
    ...
    { title: 'Rates', path: '/rates', color: 'orange', roles: ['admin', 'operator'], description: 'Rate table by route with weight-break pricing',
    ...
    { title: 'GHA Profiles', path: '/gha', color: 'pink', roles: ['admin', 'operator'], description: 'Ground Handling Agents: names, abbreviations and locations',
    ...
    { title: 'IATA Airports', path: '/airports', color: 'slate', roles: ['admin', 'operator'], description: 'IATA airport codes for routes and autocomplete',
    ...
    { title: 'Users', path: '/users', color: 'lime', roles: ['admin'], description: 'System user management with roles and permissions',
  ...
  const modules = allModules.filter(m => !m.roles || m.roles.includes(userRole));
  ...
  return (
    <Layout>
      <div className="page-wrapper">
        ...
        {/* AWB stock alert banner (admin only) */}
        {isAdmin && awbAlertCount > 0 && (
          <div
            onClick={() => navigate('/awb-stock')}
            style={{ cursor: 'pointer', background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 'var(--radius-md)', padding: 'var(--space-3) var(--space-4)', marginBottom: 'var(--space-6)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
          >
            <span style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: '#92400e' }}>
              ⚠️ {awbAlertCount} AWB stock allocation{awbAlertCount !== 1 ? 's' : ''} running low or exhausted
            </span>
            <span style={{ fontSize: '0.75rem', color: '#92400e', textDecoration: 'underline' }}>View AWB Stock →</span>
          </div>
        )}
